...
cd $script_dir
cfg=/home/egge/gdrive/dev/packages-tiny/packages-yolov4-512-tiny-detector.cfg
weights=/home/egge/gdrive/dev/packages-tiny/packages-yolov4-512-tiny-detector_best.weights
labels=/home/egge/gdrive/dev/packages-tiny/obj.names
...
then
  echo "Updating $onnx"
  python3 ./darknet2onnx.py <(sed -e 's/^height=.*/height=512/' -e 's/^width=.*/width=512/' $cfg) $weights $onnx
  [ -e ${onnx}.engine ] && rm ${onnx}.engine
  cp -p $labels /home/egge/detector/simplescan/vehicle-labels.txt
  echo "Updated $onnx"
fi

cfg=/home/egge/gdrive/dev/ipcams/ipcams-yolov4-512-tiny-detector.cfg
...
then
  ...
  [ -e ${onnx}.engine ] && rm ${onnx}.engine
  cp -p $labels /home/egge/detector/simplescan/ipcams-labels.txt
  echo "Updated $onnx"
fi
