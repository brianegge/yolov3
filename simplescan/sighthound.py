...
def enrich(image_bytes, save_json=None):
    ...
    for o in result["objects"]:
        ...
        if annotations["recognitionConfidence"] > 0.0:
            if "attributes" in annotations:
                ...
                if (
                    "make" in system
                    and "model" in system
                    and system["make"]["confidence"] > 0.6
                ):
                    message += (
                        system["make"]["name"] + " " + system["model"]["name"] + " "
                    )
        ...
        if "licenseplate" in annotations:
            ...
            if (
                annotations["licenseplate"]["attributes"]["system"]["region"][
                    "confidence"
                ]
                > 0.55
            ):
                region = annotations["licenseplate"]["attributes"]["system"]["region"][
                    "name"
                ]
                plate["region"] = region
